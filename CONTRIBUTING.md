...
- Все вложения хранить в `99_Assets/<раздел>/`, а не в корне vault.
- Для статьи из `10_AKUZ` использовать `99_Assets/10_AKUZ/`, для `06_Onboarding` — `99_Assets/06_Onboarding/` и т.д.
- Не оставлять файлы с именами вида `Pasted image ...` в корне репозитория.

Рекомендуемая настройка Obsidian:

1. `Settings -> Files and links`
2. `Default location for new attachments` -> `In the folder specified below`
3. `Attachment folder path` -> `99_Assets`

Важно: Obsidian штатно не выбирает подпапку по разделу автоматически. Поэтому:

- либо после вставки переносить файл вручную в `99_Assets/<раздел>/`;
- либо сразу вставлять изображение в заметку, открыв целевую подпапку и затем поправляя путь ссылки.
